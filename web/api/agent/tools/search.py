...
from caelum_platform.web.api.agent.tools.utils import (
...
class Search(Tool):
    ...
    async def _call(
            ...
            if len(answer_values) > 0:
                snippets.append(
                    CitedSnippet(
                        ...
                        text="\n".join(answer_values),
                        url=f"https://www.google.com/search?q={quote(input_str)}",
                    )
                )
        ...
        return await summarize_with_sources(
            model=self.model,
            goal=goal,
            task=task,
